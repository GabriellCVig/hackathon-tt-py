...
\n    def calculate_overall_performance(self, positions):
        ...
        for currentPosition in [x for x in positions if (lambda x: include_in_total_asset_value)(x)]:
            ...
            if current_position.gross_performance:
                ...
                if not (current_position.quantity == 0):
                    has_errors = True
...
# Clone orders to keep the original values in this.orders        orders = clone_deep([x for x in self.activities if (lambda x: symbol_profile.symbol == symbol)(x)])
        is_cash = orders[0].symbol_profile.asset_sub_class == 'CASH'
        if :
            return {"current_values": {}, "current_values_with_currency_effect": {}, "fees_with_currency_effect": Decimal(0), "gross_performance": Decimal(0), "gross_performance_percentage": Decimal(0), "gross_performance_percentage_with_currency_effect": Decimal(0), "gross_performance_with_currency_effect": Decimal(0), "has_errors": False, "initial_value": Decimal(0), "initial_value_with_currency_effect": Decimal(0), "investment_values_accumulated": {}, "investment_values_accumulated_with_currency_effect": {}, "investment_values_with_currency_effect": {}, "net_performance": Decimal(0), "net_performance_percentage": Decimal(0), "net_performance_percentage_with_currency_effect_map": {}, "net_performance_values": {}, "net_performance_values_with_currency_effect": {}, "net_performance_with_currency_effect_map": {}, "time_weighted_investment": Decimal(0), "time_weighted_investment_values": {}, "time_weighted_investment_values_with_currency_effect": {}, "time_weighted_investment_with_currency_effect": Decimal(0), "total_account_balance_in_base_currency": Decimal(0), "total_dividend": Decimal(0), "total_dividend_in_base_currency": Decimal(0), "total_interest": Decimal(0), "total_interest_in_base_currency": Decimal(0), "total_investment": Decimal(0), "total_investment_with_currency_effect": Decimal(0), "total_liabilities": Decimal(0), "total_liabilities_in_base_currency": Decimal(0)}
...
# For BUY / SELL activities with a MANUAL data source where no historical market price is available,# the calculation should fall back to using the activity’s unit price.            unit_price_at_end_date = latest_activity.unit_price
        ...
        if not unit_price_at_end_date or (not unit_price_at_start_date and is_before(date_of_first_transaction, start)):
            return {"current_values": {}, "current_values_with_currency_effect": {}, "fees_with_currency_effect": Decimal(0), "gross_performance": Decimal(0), "gross_performance_percentage": Decimal(0), "gross_performance_percentage_with_currency_effect": Decimal(0), "gross_performance_with_currency_effect": Decimal(0), "has_errors": True, "initial_value": Decimal(0), "initial_value_with_currency_effect": Decimal(0), "investment_values_accumulated": {}, "investment_values_accumulated_with_currency_effect": {}, "investment_values_with_currency_effect": {}, "net_performance": Decimal(0), "net_performance_percentage": Decimal(0), "net_performance_percentage_with_currency_effect_map": {}, "net_performance_with_currency_effect_map": {}, "net_performance_values": {}, "net_performance_values_with_currency_effect": {}, "time_weighted_investment": Decimal(0), "time_weighted_investment_values": {}, "time_weighted_investment_values_with_currency_effect": {}, "time_weighted_investment_with_currency_effect": Decimal(0), "total_account_balance_in_base_currency": Decimal(0), "total_dividend": Decimal(0), "total_dividend_in_base_currency": Decimal(0), "total_interest": Decimal(0), "total_interest_in_base_currency": Decimal(0), "total_investment": Decimal(0), "total_investment_with_currency_effect": Decimal(0), "total_liabilities": Decimal(0), "total_liabilities_in_base_currency": Decimal(0)}
# Add a synthetic order at the start and the end date        orders.append({"date": start_date_string, "fee": Decimal(0), "fee_in_base_currency": Decimal(0), "item_type": 'start', "quantity": Decimal(0), "symbol_profile": {"data_source": data_source, "symbol": symbol, "asset_sub_class": 'CASH' if is_cash else None}, "type": 'BUY', "unit_price": unit_price_at_start_date})
        orders.append({"date": end_date_string, "fee": Decimal(0), "fee_in_base_currency": Decimal(0), "item_type": 'end', "symbol_profile": {"data_source": data_source, "symbol": symbol, "asset_sub_class": 'CASH' if is_cash else None}, "quantity": Decimal(0), "type": 'BUY', "unit_price": unit_price_at_end_date})
        ...
        for dateString in self.chart_dates:
            ...
            if :
                ...
                orders.append({"date": date_string, "fee": Decimal(0), "fee_in_base_currency": Decimal(0), "quantity": Decimal(0), "symbol_profile": {"data_source": data_source, "symbol": symbol, "asset_sub_class": 'CASH' if is_cash else None}, "type": 'BUY', "unit_price": market_symbol_map[date_string][symbol] or last_unit_price, "unit_price_from_market_data": market_symbol_map[date_string][symbol] or last_unit_price})
            latest_activity = orders.at(-1)
            last_unit_price = latest_activity.unit_price_from_market_data or latest_activity.unit_price
# Sort orders so that the start and end placeholder order are at the correct# position        orders = sort_by(orders, lambda x: True  # TODO: complex arrow function)
        index_of_start_order = orders.find_index(lambda x: item_type == 'start')
        index_of_end_order = orders.find_index(lambda x: item_type == 'end')
...
# Take the unit price of the order as the market price if there are no# orders of this symbol before the start date            order.unit_price = orders[].unit_price if index_of_start_order == 0 else unit_price_at_start_date
        ...
        market_price_in_base_currency = (order.unit_price_from_market_data * current_exchange_rate or 1) or Decimal(0)
        market_price_in_base_currency_with_currency_effect = (order.unit_price_from_market_data * exchange_rate_at_order_date or 1) or Decimal(0)
        ...
        if not investment_at_start_date and :
            investment_at_start_date = total_investment or Decimal(0)
            investment_at_start_date_with_currency_effect = total_investment_with_currency_effect or Decimal(0)
        ...
        if (total_units == 0):
# Reset tracking variables when position is fully closed            total_investment_from_buy_transactions = Decimal(0)
            total_investment_from_buy_transactions_with_currency_effect = Decimal(0)
            total_quantity_from_buy_transactions = Decimal(0)
        if portfolio_calculator.enable_logging:
        ...
        if :
# Only consider periods with an investment for the calculation of# the time weighted investment            if (value_of_investment_before_transaction > 0) and 'BUY' 'SELL'.includes(order.type):
...
# Sum up the total investment days since the start date to calculate# the time weighted investment                total_investment_days days_since_last_order
            ...
            investment_values_with_currency_effect[order.date] = ((investment_values_with_currency_effect[order.date] or Decimal(0)) + transaction_investment_with_currency_effect)
# If duration is effectively zero (first day), use the actual investment as the base.# Otherwise, use the calculated time-weighted average.            time_weighted_investment_values[order.date] = (sum_of_time_weighted_investments / total_investment_days) if  else total_investment if (total_investment > 0) else Decimal(0)
            time_weighted_investment_values_with_currency_effect[order.date] = (sum_of_time_weighted_investments_with_currency_effect / total_investment_days) if  else total_investment_with_currency_effect if (total_investment_with_currency_effect > 0) else Decimal(0)
        if portfolio_calculator.enable_logging:
        ...
        time_weighted_average_investment_between_start_and_end_date = (sum_of_time_weighted_investments / total_investment_days) if  else Decimal(0)
        time_weighted_average_investment_between_start_and_end_date_with_currency_effect = (sum_of_time_weighted_investments_with_currency_effect / total_investment_days) if  else Decimal(0)
        ...
        for dateRange in '1d' '1y' '5y' 'max' 'mtd' 'wtd' 'ytd' *[(lambda x: format(date, 'yyyy'))(x) for x in [x for x in each_year_of_interval({"end": end, "start": start}) if (lambda x: not is_this_year(date))(x)]]:
            ...
            current_values_at_date_range_start_with_currency_effect = current_values_with_currency_effect[range_start_date_string] or Decimal(0)
            investment_values_accumulated_at_start_date_with_currency_effect = investment_values_accumulated_with_currency_effect[range_start_date_string] or Decimal(0)
 ...
 i 1             date = self.chart_dates[i]
            if :
                ...
                if :
                    break
            if  and (investment_values_accumulated_with_currency_effect[date] > 0):
            ...
            net_performance_with_currency_effect_map[date_range] = (net_performance_values_with_currency_effect[range_end_date_string] - # If the date range is 'max', take 0 as a start value. Otherwise,, # the value of the end of the day of the start date is taken which, # differs from the buying price., Decimal(0) if date_range == 'max' else (net_performance_values_with_currency_effect[range_start_date_string] or Decimal(0))) or Decimal(0)
